...
class Matrix:
    def __init__(self, app, font_size=8):
        ...
        self.katakana = np.array([chr(int('0x30a0', 16) + i) for i in range(96)] + ['' for i in range(15)])
...
class MatrixVision:
    def __init__(self):
        ...
        pygame.camera.init()
        self.cam = pygame.camera.Camera(pygame.camera.list_cameras()[0],self.RES)
        self.cam.start()
    ...
    def run(self):
        while True:
            self.draw()
            [exit(0) for i in pg.event.get() if i.type == pg.QUIT]
            pg.display.flip()
