...
class RandomSLE:
    ...
    def __init__(self,n):
        ...
        self.__matrix = [[(round(-n+random.random()*2*n,2)) for j in range(n)] for i in range(n)]
        #решения Х
        self.__solution = [round(-n+random.random()*2*n,2) for i in range(n)]
        #нахождение f
        self.__result = [sum([self.__matrix[i][j] * self.__solution[j] for j in range(n)]) for i in range(n)]
